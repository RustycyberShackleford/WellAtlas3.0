...
def job_detail(site_id,job_id):
    ...
    cur.execute("SELECT sites.*, customers.name as customer FROM sites JOIN customers ON customers.id=sites.customer_id WHERE sites.id=?", (site_id,))
    site=cur.fetchone()
    cur.execute(f"SELECT * FROM job_notes WHERE job_id=? ORDER BY datetime(created_at) {'ASC' if order=='asc' else 'DESC'}", (job_id,))
    notes=[dict(r) for r in cur.fetchall()]; c.close()
    return render_template("job_detail.html", site=dict(site), job=dict(job), notes=notes)

# API: create site, search, cascades
@app.route("/api/site_create", methods=["POST"])
def api_site_create():
...
def api_sites():
    ...
    c=db();cur=c.cursor()
    sql = "SELECT DISTINCT sites.*, customers.name as customer FROM sites JOIN customers ON customers.id=sites.customer_id LEFT JOIN jobs ON jobs.site_id = sites.id"
    params = []; wh = []
    if customer: wh.append("customers.name = ?"); params.append(customer)
    if q:
        like = f"%{q}%"
        wh.append("(customers.name LIKE ? OR sites.name LIKE ? OR sites.description LIKE ? OR jobs.job_number LIKE ? OR jobs.description LIKE ? OR EXISTS(SELECT 1 FROM job_notes jn JOIN jobs j2 ON j2.id=jn.job_id AND j2.site_id=sites.id WHERE jn.body LIKE ?))")
